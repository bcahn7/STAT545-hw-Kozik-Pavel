...
```{r}
vc1 <- ggplot(gapminder, aes(x = year, y = lifeExp, color = continent)) +
geom_point() 
vc1 + geom_smooth(se=FALSE)
vc1 + geom_smooth(method="lm")
```
...
```{r}
vc1 <- ggplot(gapminder, aes(year, lifeExp, color = continent)) 
vc1 + geom_smooth(se=FALSE)
vc1 + geom_smooth(method="lm") +
geom_point() 

##This changes the layering, this puts the points on top of the smoother, before we had the smoother on top of the points
```
...
ggplot(gapminder, aes(x = year, y = lifeExp)) +
  facet_wrap(~ continent) +
  geom_point() +
  geom_smooth(method = 'loess', se = "False", span = 4.5) +  #this creates a loess line
  geom_smooth(method= "lm", color = "red") # this creates a linear line
```
...
```{r}
gapminder %>%
  mutate(Canada = ifelse(country=="Canada", "yes", "no")) %>%
  ggplot(aes(x=year, y=lifeExp, colour=Canada, group=country)) +
  geom_line(size=2) +
  scale_colour_manual(values = c("black", "red")) +
  geom_line(aes(x=year, y=lifeExp), data=subset(gapminder, country =="Canada"), colour = "red")
```
